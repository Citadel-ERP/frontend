...
export const Overview: React.FC<OverviewProps> = ({ employee, employeeDetails, token, onRefresh }) => {
  ...
  const [editData, setEditData] = useState({
    earned_leaves: employee.earned_leaves,
    sick_leaves: employee.sick_leaves,
  ...
  const markGiftBasketSent = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/hr_manager/markGiftBasketSent`, {
        method: 'POST',
  ...
  const handleSpecialAttendance = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/hr_manager/allowSpecialAttendance`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          token,
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await fetch(`${BACKEND_URL}/hr_manager/unlockDevice`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  token,
  ...
  const handleEditEmployee = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/hr_manager/updateEmployee`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          token,
  ...
  const uploadPayslip = async (month: string, year: string, fileUri: string) => {
    // Implementation for payslip upload
    Alert.alert('Info', 'Payslip upload functionality to be implemented');
  };

  const downloadAttendance = async (month: string, year: string) => {
    try {
  ...
  return (
    ...
    >
          ...
          <View style={styles.actionMenu}>
            {actionMenuItems.map((item, index) => (
              <TouchableOpacity
                ...
                onPress={item.action}
              >
                <Ionicons name={item.icon as any} size={20} color={WHATSAPP_COLORS.textPrimary} />
      ...
      <View style={styles.infoCardsContainer}>
        ...
        <View style={styles.infoCard}>
          <View style={styles.infoCardHeader}>
            ...
            <Text style={styles.infoCardTitle}>Payslip & Attendance</Text>
          </View>
          
          <View style={styles.payslipContainer}>
            <TouchableOpacity style={styles.payslipButton}>
              <Ionicons name="cloud-upload-outline" size={20} color={WHATSAPP_COLORS.primary} />
              <Text style={styles.payslipButtonText}>Upload Payslip</Text>
            </TouchableOpacity>
            
            <TouchableOpacity style={styles.payslipButton}>
              <Ionicons name="download-outline" size={20} color={WHATSAPP_COLORS.primary} />
              <Text style={styles.payslipButtonText}>Download Attendance</Text>
            </TouchableOpacity>
          </View>
        </View>

        {!employee.gift_basket_sent && (
          <TouchableOpacity
            style={styles.giftBasketButton}
            onPress={markGiftBasketSent}
          >
            <Ionicons name="gift-outline" size={24} color="#fff" />
            <Text style={styles.giftBasketButtonText}>Mark Gift Basket Sent</Text>
          </TouchableOpacity>
        )}

        <TouchableOpacity
          style={styles.offboardButton}
          onPress={() => setShowOffboardingModal(true)}
        >
          <Ionicons name="log-out-outline" size={24} color="#fff" />
          <Text style={styles.offboardButtonText}>Offboard Employee</Text>
        </TouchableOpacity>
      </View>
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setShowEditEmployeeModal(false)}
      >
        <View style={styles.modalOverlay}>
          <ScrollView style={styles.modalContainer}>
            ...
            <View style={styles.editForm}>
              ...
              <TextInput
                ...
                value={editData.earned_leaves.toString()}
                onChangeText={(text) => setEditData({...editData, earned_leaves: parseInt(text) || 0})}
              ...
              <TextInput
                style={styles.editInput}
                value={editData.sick_leaves.toString()}
                onChangeText={(text) => setEditData({...editData, sick_leaves: parseInt(text) || 0})}
                keyboardType="numeric"
              />
              ...
              <TextInput
                style={styles.editInput}
                value={editData.casual_leaves.toString()}
                onChangeText={(text) => setEditData({...editData, casual_leaves: parseInt(text) || 0})}
                keyboardType="numeric"
              />
              ...
              <TextInput
                ...
                onChangeText={(text) => setEditData({...editData, login_time: text})}
                placeholder="HH:MM"
              />
              ...
              <TextInput
                style={styles.editInput}
                value={editData.logout_time}
                onChangeText={(text) => setEditData({...editData, logout_time: text})}
                placeholder="HH:MM"
              />
            </View>
