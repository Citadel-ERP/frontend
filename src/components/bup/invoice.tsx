...
const Invoice: React.FC<InvoiceProps> = ({
  ...
  const fetchInvoice = async () => {
    if (!token) {
      onBack();
      setTimeout(() => Alert.alert('Error', 'Authentication token not found'), 300);
      return;
    }
    try {
      setLoading(true);
      ...
      if (!data.invoices || data.invoices.length === 0) {
        onBack();
        setTimeout(() => Alert.alert('Invoice Not Found', 'No invoice found for this lead'), 300);
        return;
      }

      setInvoice({ ...data.invoices[0], files: data.invoices[0].files || [] });
      setLoading(false);
    } catch (error) {
      console.error('Error fetching invoice:', error);
      onBack();
      setTimeout(() => Alert.alert('Error', 'Failed to fetch invoice. Please try again.'), 300);
    }
  };
      ...
      <View style={s.container}>
        <SafeAreaView style={s.headerSafeArea} edges={['top']}>
          <View style={s.header}>
            ...
            <TouchableOpacity onPress={onBack} style={s.backButton}><BackIcon /></TouchableOpacity>
  ...
  return (
    <View style={s.container}>
      ...
      <ScrollView style={s.scrollView} contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {((invoice.executive_name?.trim()) || (invoice.particular_matter_to_mention?.trim())) && (
          <View style={s.card}>
            <Text style={s.sectionTitle}>Additional Information</Text>
            {invoice.executive_name?.trim() && (
              <InfoRow label="Executive Name" value={invoice.executive_name} icon="person" />
            )}
            {invoice.particular_matter_to_mention?.trim() && (
              <View style={s.infoRow}>
...
const s = StyleSheet.create({
  ...
  infoValue:   { fontSize: 15, color: C.textPrimary, lineHeight: 20 },
  fileItem: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: C.background, padding: 12, borderRadius: 8, marginBottom: 8,
  },
  fileIconContainer: {
