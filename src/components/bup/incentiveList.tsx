 ...
 * Tapping a row opens the existing Incentive management screen for that
 * incentive_id. The "+" button in the header starts the create flow.
 *
 * Design mirrors InvoiceList for visual consistency.
 *
 * CHANGES:
 *  - Row title now shows "Incentive #1", "#2", etc. based on position
 *    in the list (oldest first = #1), NOT the database id.
 */
...
interface IncentiveListProps {
  ...
  theme: ThemeColors;
  onBack: () => void;
  /** Callback with selected incentive_id → parent opens Incentive management */
  onSelectIncentive: (incentiveId: number) => void;
  /** Callback to open the create-incentive flow */
  onCreateIncentive: () => void;
  /** Whether the "+" button should be shown (backend-controlled) */
  canCreate?: boolean;
}
...
const IncentiveList: React.FC<IncentiveListProps> = ({
  ...
  const fetchIncentives = useCallback(async () => {
    ...
    try {
      ...
      // Backend returns { incentives: [...] } when queried by lead_id
  ...
  const renderRow = ({ item, index }: { item: IncentiveSummary; index: number }) => {
    const statusColor = STATUS_COLOR[item.status] ?? C.textSecondary;
    const statusLabel = STATUS_LABEL[item.status] ?? item.status;

    // Human-friendly numbering: oldest incentive in the list = #1
