...
// ─── Colour palette (matches LeadDetails / Invoice) ───────────────────────
const C = {
  primary:       '#075E54',
  primaryLight:  '#128C7E',
  primaryDark:   '#054D44',
...
const InvoiceList: React.FC<InvoiceListProps> = ({
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        // 404 → no invoices yet; that's fine when canCreate === true
        setInvoices([]);
        setLoading(false);
        return;
      }
  ...
  const renderRow = ({ item, index }: { item: InvoiceData; index: number }) => (
    ...
    >
      ...
      <View style={s.rowBody}>
        {/* Use index + 1 for a per-lead sequential number instead of the DB id */}
        <Text style={s.rowTitle} numberOfLines={1}>
          Invoice #{index + 1} — {item.vendor_name || 'Unknown Vendor'}
        </Text>
        <Text style={s.rowMeta} numberOfLines={1}>
          {item.property_type}  ·  {item.billing_area}
        </Text>
    ...
    return (
      <View style={s.emptyWrap}>
        ...
        <Text style={s.emptyText}>
          {canCreate
            ? 'Tap the "+" button above to raise the first invoice for this lead.'
    ...
    <View style={s.root}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={
            invoices.length > 0 ? (
              <Text style={s.listCount}>{invoices.length} invoice{invoices.length !== 1 ? 's' : ''}</Text>
            ) : null
          }
        />
      )}
