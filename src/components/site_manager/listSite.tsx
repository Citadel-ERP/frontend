...
const ListSite: React.FC<ListSiteProps> = ({
  ...
  // Effect to handle queued modals after filter modal closes
  useEffect(() => {
  ...
  // Effect to reopen filter modal after nested modal closes
  useEffect(() => {
  ...
  const handleOpenNestedFilter = useCallback((
    filterType: 'property_type' | 'building_status' | 'floor_condition' | 'micro_market'
  ) => {
          ...
          onPress: async () => {
            ...
            try {
              ...
              const response = await fetch(`${BACKEND_URL}/manager/deleteSite`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ token, site_ids: selectedSites }),
              });
              if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
              const data = await response.json();
              if (data.message !== 'Site deleted successfully') throw new Error(data.message || 'Failed to delete sites');
  ...
  const renderSiteItem = useCallback((item: Site, index: number) => {
    ...
    const handleCardPress = () => {
      if (selectionMode) handleSiteSelection(item.id);
      else onSitePress(item);
    };

    return (
      <View key={item.id} style={[styles.siteCard, isSelected && styles.siteCardSelected]}>
        {selectionMode && (
        ...
        <View style={styles.cardContent}>
          <TouchableOpacity onPress={handleCardPress} onLongPress={() => handleLongPress(item.id)} activeOpacity={0.7}>
            <View style={styles.cardHeader}>
              <View style={styles.headerLeft}>
                <Text style={styles.siteName} numberOfLines={1}>{item.building_name || 'Unnamed Site'}</Text>
              </View>
              {!selectionMode && (
                <TouchableOpacity
                  style={styles.editIconButton}
                  onPress={(e) => { e.stopPropagation(); onEditSite(item); }}
          ...
          >
            ...
            {item.total_floors && (
              <View style={[styles.detailChip, { backgroundColor: WHATSAPP_COLORS.lightYellow }]}>
                ...
                <Text style={[styles.detailChipText, { color: '#D97706' }]}>{item.total_floors} floors</Text>
              </View>
            )}
            {item.oc && (
    ...
    selectedSites, selectionMode, onSitePress, onEditSite,
    handleSiteSelection, handleLongPress, getStatusColor, getPropertyType, beautifyName,
  ]);
  ...
  }, [loading, searchTags, activeFilterCount, onCreateSite]);

  // Render Filter Dropdown (nested modal)
  const renderFilterDropdown = useCallback((
    ...
    >
      <TouchableOpacity style={styles.dropdownOverlay} activeOpacity={1} onPress={handleCloseNestedFilter}>
        <View style={styles.dropdownContainer}>
          ...
          <View style={styles.nestedFilterFooter}>
            <TouchableOpacity style={styles.nestedFilterDoneButton} onPress={handleCloseNestedFilter}>
      ...
      <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowFilterModal(false)}>
        <View style={styles.filterModalContent}>
          ...
          <ScrollView style={styles.filterList}>
            {/* Property Type */}
            <TouchableOpacity style={styles.filterOption} onPress={() => handleOpenNestedFilter('property_type')}>
            ...
            {/* Building Status */}
            <TouchableOpacity style={styles.filterOption} onPress={() => handleOpenNestedFilter('building_status')}>
              <View style={styles.filterOptionLeft}>
            ...
            {/* Floor Condition */}
            <TouchableOpacity style={styles.filterOption} onPress={() => handleOpenNestedFilter('floor_condition')}>
              <View style={styles.filterOptionLeft}>
  ...
  ), [showFilterModal, localFilters, clearFilters, applyFilters, handleOpenNestedFilter, toggleOcFilter]);
    ...
    >
      <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowActionsModal(false)}>
        <View style={styles.actionsModal}>
          <Text style={styles.actionsModalTitle}>{selectedSites.length} site(s) selected</Text>
          <TouchableOpacity style={[styles.actionOption, styles.actionOptionDanger]} onPress={handleBulkDelete}>
            <View style={styles.actionOptionLeft}>
              <Ionicons name="trash-outline" size={22} color={WHATSAPP_COLORS.danger} />
              <Text style={[styles.actionOptionText, styles.actionOptionTextDanger]}>Delete Sites</Text>
            </View>
            <Ionicons name="chevron-forward" size={20} color={WHATSAPP_COLORS.textTertiary} />
          </TouchableOpacity>
          <TouchableOpacity style={styles.cancelActionButton} onPress={() => setShowActionsModal(false)}>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        <View style={styles.searchInputWrapper}>
          ...
          <View style={styles.searchActions}>
            ...
            {selectionMode && (
              <TouchableOpacity style={styles.actionIconButton} onPress={() => setShowActionsModal(true)}>
      ...
      {selectionMode && (
        <View style={styles.selectionBar}>
          <Text style={styles.selectionText}>{selectedSites.length} site(s) selected</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            ...
            {localFilters.micro_market.map((value) => {
              ...
              return (
                <View key={`micro-${value}`} style={styles.activeFilter}>
                  <Text style={styles.activeFilterText}>{option?.label || value}</Text>
                  <TouchableOpacity onPress={() => removeFilter('micro_market', value)}>
                    <Ionicons name="close" size={14} color={WHATSAPP_COLORS.primary} />
                  </TouchableOpacity>
                </View>
              );
            ...
            {localFilters.oc === true && (
              <View style={styles.activeFilter}>
                ...
                <TouchableOpacity onPress={removeOcFilter}>
                  <Ionicons name="close" size={14} color={WHATSAPP_COLORS.primary} />
                </TouchableOpacity>
              </View>
            )}
      ...
      {/* Nested Filter Dropdowns */}
      {renderFilterDropdown(
        'property_type', 'Select Property Type', PROPERTY_TYPE_OPTIONS,
        localFilters.property_type,
        (value) => {
      ...
      {renderFilterDropdown(
        'building_status', 'Select Building Status', BUILDING_STATUS_OPTIONS,
      ...
      {renderFilterDropdown(
        'floor_condition', 'Select Floor Condition', FLOOR_CONDITION_OPTIONS,
...
const styles = StyleSheet.create({
  ...
  searchInput: {
    ...
    borderRadius: 20, paddingHorizontal: 45, paddingVertical: 12,
    fontSize: 16, flex: 1, color: WHATSAPP_COLORS.textPrimary,
  },
  searchIcon: { position: 'absolute', left: 15, zIndex: 1 },
  addSearchButton: { position: 'absolute', right: 50, zIndex: 1, padding: 4 },
  searchActions: { position: 'absolute', right: 15, zIndex: 1, flexDirection: 'row', gap: 8 },
  filterButton: { padding: 8 },
  actionIconButton: { padding: 8 },
  filterBadge: {
    position: 'absolute', top: -2, right: -2,
    backgroundColor: WHATSAPP_COLORS.danger, borderRadius: 10,
  ...
  selectionText: { fontSize: 15, fontWeight: '600', color: WHATSAPP_COLORS.info },
  cancelSelectionText: { fontSize: 15, fontWeight: '600', color: WHATSAPP_COLORS.danger },
  searchTagsContainer: {
  ...
  siteCard: {
    backgroundColor: WHATSAPP_COLORS.surface, borderRadius: 16, padding: 16, marginBottom: 12,
  ...
  selectionCheckbox: { position: 'absolute', top: 12, left: 12, zIndex: 10 },
  checkbox: {
  ...
  propertyTypeBadge: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  propertyTypeText: { fontSize: 12, fontWeight: '700', color: WHATSAPP_COLORS.textPrimary },
  editIconButton: { padding: 8, borderRadius: 10, backgroundColor: WHATSAPP_COLORS.primaryLight + '15' },
  ...
  pricingBadge: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: WHATSAPP_COLORS.lightGreen, paddingHorizontal: 12, paddingVertical: 8,
    borderRadius: 10, gap: 8, alignSelf: 'flex-start',
  },
  pricingText: { fontSize: 16, color: WHATSAPP_COLORS.success, fontWeight: '700' },
  loadingContainer: { alignItems: 'center', justifyContent: 'center', paddingVertical: 60 },
  loadingText: { marginTop: 12, fontSize: 14, color: WHATSAPP_COLORS.textSecondary, fontWeight: '500' },
  ...
  emptyStateTitle: { fontSize: 20, fontWeight: '700', color: WHATSAPP_COLORS.textPrimary, marginBottom: 8 },
  emptyStateText: { fontSize: 15, color: WHATSAPP_COLORS.textSecondary, textAlign: 'center', lineHeight: 22, marginBottom: 24 },
  createButton: {
    backgroundColor: WHATSAPP_COLORS.primary, paddingHorizontal: 24, paddingVertical: 14,
    borderRadius: 12, flexDirection: 'row', alignItems: 'center', gap: 8,
    shadowColor: WHATSAPP_COLORS.primary, shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.3, shadowRadius: 8, elevation: 4,
  },
  createButtonText: { color: WHATSAPP_COLORS.white, fontSize: 16, fontWeight: '700' },
  loadMoreButton: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    paddingVertical: 16, gap: 8, backgroundColor: WHATSAPP_COLORS.surface,
    borderRadius: 12, marginTop: 4, marginBottom: 20,
  ...
  ocToggleThumb: {
    ...
    backgroundColor: WHATSAPP_COLORS.white, alignSelf: 'flex-start',
  },
  ...
  actionsModal: { backgroundColor: WHATSAPP_COLORS.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20 },
  actionsModalTitle: { fontSize: 18, fontWeight: '700', color: WHATSAPP_COLORS.textPrimary, marginBottom: 20, textAlign: 'center' },
  actionOption: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingVertical: 16, paddingHorizontal: 16,
    backgroundColor: WHATSAPP_COLORS.backgroundSecondary, borderRadius: 12, marginBottom: 12,
  },
  actionOptionDanger: { backgroundColor: WHATSAPP_COLORS.lightRed },
  actionOptionLeft: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  actionOptionText: { fontSize: 16, fontWeight: '600', color: WHATSAPP_COLORS.textPrimary },
  actionOptionTextDanger: { color: WHATSAPP_COLORS.danger },
  cancelActionButton: { paddingVertical: 16, alignItems: 'center', marginTop: 8 },
  cancelActionText: { fontSize: 16, color: WHATSAPP_COLORS.textSecondary, fontWeight: '600' },
  dropdownOverlay: {
  ...
  dropdownContainer: {
    ...
    backgroundColor: WHATSAPP_COLORS.surface, borderRadius: 16, padding: 16, maxHeight: '60%',
  },
  ...
  nestedFilterFooter: { marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: WHATSAPP_COLORS.border },
  nestedFilterDoneButton: { backgroundColor: WHATSAPP_COLORS.primary, paddingVertical: 14, alignItems: 'center', borderRadius: 8 },
  nestedFilterDoneText: { fontSize: 16, color: WHATSAPP_COLORS.white, fontWeight: '600' },
  loadingOverlay: {
    position: 'absolute', top: 0, left: 0, right: 0, bottom: 0,
    backgroundColor: WHATSAPP_COLORS.overlay, justifyContent: 'center', alignItems: 'center', zIndex: 1000,
  },
  loadingCard: {
    backgroundColor: WHATSAPP_COLORS.surface, padding: 24, borderRadius: 16, alignItems: 'center',
    shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8,
  },
  loadingOverlayText: { marginTop: 16, fontSize: 16, color: WHATSAPP_COLORS.textPrimary, fontWeight: '600' },
});
