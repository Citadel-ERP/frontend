// share.tsx - Updated with Group Support
...
export const ShareScreen: React.FC<ShareScreenProps> = ({
  ...
  const fetchUsersAndGroups = useCallback(async (search: string = '') => {
    if (isLoading) return;
    ...
    try {
      ...
      if (isMountedRef.current) {
        ...
        Alert.alert(
          ...
          [{ text: 'Retry', onPress: () => fetchUsersAndGroups(search) }]
        );
      }
    } finally {
      if (isMountedRef.current) {
  ...
  const handleUserToggle = useCallback((userId: string) => {
    setSelectedUserIds(prev => {
      ...
      if (wasSelected) {
        ...
        // Show group options if crossing 2-recipient threshold (users + groups)
        const totalSelected = next.size + selectedGroupIds.size;
  ...
  const handleGroupToggle = useCallback((groupId: string) => {
    setSelectedGroupIds(prev => {
      ...
      if (next.has(groupId)) {
        ...
        next.add(groupId);
        // Groups are already groups, so no need for group creation option
      }
  ...
  const handleSelectAll = useCallback(() => {
    ...
    if (totalSelected === totalItems) {
      ...
      // Select all
      const allUserIds = new Set(users.map(user => user.employee_id || user.id?.toString() || ''));
  ...
  }, [users, groups, selectedUserIds.size, selectedGroupIds.size, hasChosenShareType]);
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      const shareData: any = {
        ...
        create_group: createAsGroup && selectedGroupIds.size === 0, // Only for user-to-user
      };
      ...
      return (
        <View style={styles.emptyContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => fetchUsersAndGroups(searchQuery)}
            activeOpacity={0.7}
          >
          ...
          <SectionList
            ...
            keyExtractor={(item, index) => 
              'id' in item && typeof item.id === 'string' 
                ? item.id 
                : (item as User).employee_id || (item as User).id?.toString() || index.toString()
            }
